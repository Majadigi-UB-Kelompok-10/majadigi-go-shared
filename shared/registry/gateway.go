...
func AutoRegisterFull(gatewayDBUrl, slugName, pageURL, title, iconURL, description string, categoryIDs []string) {
	...
	if err != nil {
		err = db.QueryRow(ctx, `
			SELECT service_list_id FROM service_list WHERE title = $1
		`, title).Scan(&serviceID)
		if err != nil {
			log.Printf("Gagal ambil service_list_id untuk '%s': %v\n", title, err)
			return
		}
	}

	// 3. Assign ke kategori (idempoten)
